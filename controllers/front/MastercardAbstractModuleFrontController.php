...
abstract class MastercardAbstractModuleFrontController extends \ModuleFrontController
{
    ...
    const THREEDSECUREID         = '3DSecureId';
    const THREED                 = '3DSecure';
    ...
    {
        parent::isPostProcess();

        $result = false;

        if (Tools::getValue('process_acs_result') === "1") {

            $result = $this->handleProcessAcsResult();
        
        } elseif (Tools::getValue('check_3ds_enrollment') === '2') {
        
            $result = $this->handleUpdateSession();
        
        } elseif (Tools::getValue('check_3ds_enrollment') === "1") {
    ...
    private function handleProcessAcsResult()
    {
        $paRes                = Tools::getValue('PaRes');
        $this->threeDSecureId = Tools::getValue(self::THREEDSECUREID);
        ...
        $this->threeDSecureData = array(
            'acsEci'              => $response[self::THREED]['acsEci'],
            'authenticationToken' => $response[self::THREED]['authenticationToken'],
            'paResStatus'         => $response[self::THREED]['paResStatus'],
            'veResEnrolled'       => $response[self::THREED]['veResEnrolled'],
            'xid'                 => $response[self::THREED]['xid'],
        );
    ...
    private function handleUpdateSession()
    {
        ...
        $response = $this->client->updateSession($orderId, $sessionId, $order, $auth, $transaction);
    ...
    private function handle3dsEnrollment()
    {
        ...
        $response = $this->client->check3dsEnrollment($threeD, $order, $session);
        ...
        if (isset($response[self::THREED]['authenticationRedirect'])) {
            $tdsAuth = $response[self::THREED]['authenticationRedirect']['customized'];
            $this->context->smarty->assign([
                ...
                'returnUrl'              => $this->context->link->getModuleLink(
                    ...
                    Tools::getValue('controller'),
                    [
                        '3DSecureId'         => $response[self::THREEDSECUREID],
    ...
    private function redirectToOrderPage(): string
    {
