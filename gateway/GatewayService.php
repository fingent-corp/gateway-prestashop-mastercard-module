...
namespace Fingent\Mastercard\Gateway;
...
class GatewayService
{
    ...
    const MISSING_ID     = 'Missing session or ID.';
    ...
    public function validateInitiateAuthenticationResponse($data)
    {
        if (!isset($data['result']) || $data['result'] !== 'SUCCESS') {
            throw new GatewayResponseException(self::INVALID_RESULT);
        }

        if (!isset($data['transaction']['id'])) {
            throw new GatewayResponseException(self::MISSING_ID);
        }
    }
    ...
    {
        if (!is_array($data)) {
            throw new MasterCardPaymentException('Invalid transaction response: response is not an array.');
        }
        ...
        if (!isset($data['result']) || $data['result'] !== 'SUCCESS') {
            ...
            if (isset($data['error'])) {
                $errorMessage .= self::ERROR . ($data['error']['explanation'] ?? $data['error']['cause'] ?? self::UNKNOWN);
            } elseif (isset($data['response'])) {
                $errorMessage .= self::RESPONSE . json_encode($data['response']);
            } else {
                $errorMessage .= self::UNKNOWN_ERROR;
            }

            throw new MasterCardPaymentException($errorMessage);
        }

        if (!isset($data['transaction']) || !isset($data['transaction']['id'])) {
            throw new MasterCardPaymentException('Invalid transaction response: missing transaction details.');
        }
    }
   ...
   public function validateOrderResponse($data)
    {
        if (!is_array($data)) {
            throw new MasterCardPaymentException('Invalid order response: response is not an array.');
        }
        ...
        if (!isset($data['id'])) {
            throw new MasterCardPaymentException('Invalid order response: missing order ID.');
        }
    }
    ...
    public function authenticatePayer(
        ...
        $uri     = $this->apiUrl . self::ORDER . $orderId . self::TRANSACTION . $txnId;
        $request = $this->messageFactory->createRequest('PUT', $uri);
        $stream  = $this->messageFactory->createStream(json_encode([
            ...
            'order'          => $payload['order'] ?? [],
            'billing'        => ['address' => $payload['billing'] ?? []],
            'shipping'       => [
                'address' => $payload['shipping'] ?? [],
                'contact' => $payload['shippingContact'] ?? [],
            ],
    ...
    public function createCheckoutSession(
        ...
        $request = $this->messageFactory->createRequest('POST', $uri);
        $stream  = $this->messageFactory->createStream(json_encode(array(
    ...
    public function authorize(
        ...
        $body = [
            ...
            'partnerSolutionId' => $this->getSolutionId(),
            'order'             => array_merge($payload['order'] ?? [], [
                'notificationUrl' => $this->webhookUrl,
                'reference'       => $orderId,
            ]),
            'billing'           => [
                'address' => $payload['billing'] ?? [],
            ],
            'shipping'          => [
                'address' => $payload['shipping'] ?? [],
                'contact' => $payload['shippingContact'] ?? [],
        ...
        if ($threeDSVersion === '1') {
            ...
            $body['authentication'] = $payload['authentication'] ?? [];
        } else {
            throw new MasterCardPaymentException('Invalid 3DS Version');
        }

        $request     = $this->messageFactory->createRequest('PUT', $uri);
        $stream      = $this->messageFactory->createStream(json_encode([$body]));
        $requestBody = $request->withBody($stream);
        $response    = $this->client->sendRequest($requestBody);
        $response    = json_decode($response->getBody(), true);
    ...
    {
        $uri      = $this->apiUrl.self::ORDER.$orderId;
    ...
    public function voidTxn($orderId, $txnId)
    {
        ...
        $uri      = $this->apiUrl.self::ORDER.$orderId.self::TRANSACTION.$newTxnId;
        ...
        $request     = $this->messageFactory->createRequest('PUT', $uri);
        $stream      = $this->messageFactory->createStream($requestJson);
        $requestBody = $request->withBody( $stream );
        $response    = $this->client->sendRequest($requestBody);
        $response    = json_decode($response->getBody(), true);
