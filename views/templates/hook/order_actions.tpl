  ...
  <div>
    ...
    {if $can_action}
      <div class="well hidden-print">
          {if $can_capture}
            <a id="desc-order-capture_payment" class="btn btn-default"
               href="{$link->getAdminLink('AdminMpgs')|escape:'html':'UTF-8'}&amp;action=capture&amp;id_order={$order->id|intval}">
          ...
          {if $can_void}
            <a id="desc-order-void_payment" class="btn btn-default"
               href="{$link->getAdminLink('AdminMpgs')|escape:'html':'UTF-8'}&amp;action=void&amp;id_order={$order->id|intval}">
          ...
          {if $can_refund}
            <a id="desc-order-refund_payment" class="btn btn-default"
               href="{$link->getAdminLink('AdminMpgs')|escape:'html':'UTF-8'}&amp;action=refund&amp;id_order={$order->id|intval}">
              <i class="icon-exchange"></i>
          ...
          <tr>
            ...
            <td>
                {if $refund->order_slip_id}
                  <a class="_blank" title="{l s='See the document'}"
                     href="{$link->getAdminLink('AdminPdf', true, [], ['submitAction' => 'generateOrderSlipPDF', 'id_order_slip' => $refund->order_slip_id])|escape:'html':'UTF-8'}">
